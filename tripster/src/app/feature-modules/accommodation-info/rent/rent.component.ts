...
export class RentComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.accommodationService.getAccommodation(this.id).subscribe({
			...
			error: (error: any) => {
				console.error(
					'Failed to get pricing type from the accommodation.',
					error
				);
			},
		});
	...
	dateFilter = (date: Date | null): boolean => {
		return this.calendar.some((day) => this.isSameDay(date, day.date));
	};
	...
	areDatesValid(): boolean {
		return (
			this.reservationForm.value.start < this.reservationForm.value.end
		);
	}
	...
	calculatePrice(): number {
		...
		while (checkDay < this.reservationForm.value.end) {
			...
			if (this.pricePerNight) {
				...
				if (diff)
					price = price + diff * this.reservationForm.value.guestsNo;
			}
			checkDay.setDate(checkDay.getDate() + 1);
		}
